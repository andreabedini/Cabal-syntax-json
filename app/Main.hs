...
jsonGenericPackageDescription gpd =
  JsonObject $
    concat
      [ something v (packageDescription gpd),
        jsonField "source-repos" $ JsonObject $ foldMap (something v) $ sourceRepos (packageDescription gpd),
        jsonField "custom-build" $ JsonObject $ foldMap (something v) $ setupBuildInfo (packageDescription gpd),
        jsonField "flags" $ JsonObject $ jsonGenPackageFlags v (genPackageFlags gpd),
        jsonField "library" $ JsonObject $ foldMap (jsonCondLibrary v) (condLibrary gpd),
        jsonField "other-libraries" $ JsonObject $ jsonCondLibraries v LSubLibName (condSubLibraries gpd),
        jsonField "foreign-libraries" $ JsonObject $ jsonCondForeignLibs v (condForeignLibs gpd),
        jsonField "executables" $ JsonObject $ jsonCondExecutables v (condExecutables gpd),
        jsonField "test-suites" $ JsonObject $ jsonCondTestSuites v (condTestSuites gpd),
        jsonField "benchmarks" $ JsonObject $ jsonCondBenchmarks v (condBenchmarks gpd)
      ]
  where
    v = specVersion $ packageDescription gpd

class Something a where
  something :: CabalSpecVersion -> a -> [Pair]

instance Something PackageDescription where
  something v = jsonFieldGrammar v packageDescriptionFieldGrammar

instance Something SourceRepo where
  something v repo = jsonFieldGrammar v (sourceRepoFieldGrammar (repoKind repo)) repo

instance Something SetupBuildInfo where
  something v = jsonFieldGrammar v (setupBInfoFieldGrammar False)
...
jsonCondLibrary :: CabalSpecVersion -> CondTree ConfVar c Library -> [Pair]
jsonCondLibrary v = jsonCondTree v (libraryFieldGrammar LMainLibName)

mimap ::
  (UnqualComponentName -> n) ->
  (n -> a -> [Pair]) ->
  (UnqualComponentName, a) ->
  Pair
mimap g f (n, a) = (unUnqualComponentName n, JsonObject $ f (g n) a)

jsonCondLibraries :: CabalSpecVersion -> (UnqualComponentName -> LibraryName) -> [(UnqualComponentName, CondTree ConfVar c Library)] -> [Pair]
jsonCondLibraries v f = map . mimap f $ jsonCondTree v . libraryFieldGrammar

jsonCondForeignLibs :: CabalSpecVersion -> [(UnqualComponentName, CondTree ConfVar c ForeignLib)] -> [Pair]
jsonCondForeignLibs v = map . mimap id $ jsonCondTree v . foreignLibFieldGrammar

jsonCondExecutables :: CabalSpecVersion -> [(UnqualComponentName, CondTree ConfVar c Executable)] -> [Pair]
jsonCondExecutables v = map . mimap id $ jsonCondTree v . executableFieldGrammar

jsonCondTestSuites :: CabalSpecVersion -> [(UnqualComponentName, CondTree ConfVar c TestSuite)] -> [Pair]
jsonCondTestSuites v = map . mimap id $ \_ -> jsonCondTree v testSuiteFieldGrammar . mapTreeData unvalidateTestSuite

jsonCondBenchmarks :: CabalSpecVersion -> [(UnqualComponentName, CondTree ConfVar c Benchmark)] -> [Pair]
jsonCondBenchmarks v = map . mimap id $ \_ -> jsonCondTree v benchmarkFieldGrammar . mapTreeData unvalidateBenchmark

jsonCondTree :: CabalSpecVersion -> JSONFieldGrammar s s -> CondTree ConfVar c s -> [Pair]
